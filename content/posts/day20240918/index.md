...
flowchart TD
  %% --- 定义节点样式 ---
  %% 主节点样式：外框粗、无填充色，便于突出层次名称
  classDef mainNode fill:#fff,stroke:#333,stroke-width:2px,color:#333

  %% 说明节点样式：虚线外框，字体稍小，浅灰填充区
  classDef descNode fill:#f9f9f9,stroke:#999,stroke-dasharray: 5 5,stroke-width:1px,color:#333,font-size:12px

  %% --- 五要素主流程 ---
  A(策略层):::mainNode --> B(范围层):::mainNode
  B --> C(结构层):::mainNode
  C --> D(框架层):::mainNode
  D --> E(表现层):::mainNode

  %% --- 详细说明（虚线） ---
  A -.-> A_desc[明确产品的目标、用户需求与业务需求<br/>确定产品为什么存在，以及期望实现的目标]:::descNode
  B -.-> B_desc[基于策略层，确定功能和内容需求<br/>明确要实现哪些特性和内容]:::descNode
  C -.-> C_desc[进行信息架构与交互设计<br/>将需求逻辑化、结构化，以确保产品逻辑流畅]:::descNode
  D -.-> D_desc[规划界面布局、导航设计<br/>使产品使用过程更顺畅、可用]:::descNode
  E -.-> E_desc[视觉设计与感官体验<br/>关注外观、美感和品牌风格，与整体形象保持一致]:::descNode
